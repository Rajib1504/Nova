...
export const GmailContent: React.FC<GmailContentProps> = ({
  ...
  const groupedThreads = Array.from(threadsMap.values()).map((threadEmails) => {
    ...
    const isPriority = threadEmails.some(
      (e) =>
        e.priority === "high" ||
        e.priority === "urgent" ||
  ...
  return (
    <TabsContent
      value={label}
      className="flex-1 overflow-y-auto p-4 flex flex-col gap-3 m-0 data-[state=inactive]:hidden focus-visible:outline-none relative"
    >
      {!isConnected && label === "Inbox" ? (
        <div className="flex-1 flex flex-col items-center justify-center h-full text-center mt-12 mb-12">
          ...
          <p className="text-gray-500 dark:text-gray-400 max-w-sm mb-8">
            Initialize your neural core to automatically triage, categorize, and
            organize your inbox.
          </p>
          <BlobButton
        ...
        <div className="flex-1 flex flex-col gap-3 min-h-[300px]">
          ...
          ) : groupedThreads.length === 0 && syncing ? (
            <div className="flex-1 flex flex-col items-center justify-center text-gray-500 mt-10">
              <div className="relative mb-6">
                <div className="absolute inset-0 rounded-full bg-[#FF9494] blur-xl opacity-30 animate-pulse"></div>
                <div className="relative bg-white/50 dark:bg-white/5 p-4 rounded-full border border-[#FF9494]/30 shadow-lg shadow-[#FF9494]/10">
                  <Clock className="w-10 h-10 text-[#FF9494] animate-pulse" />
                </div>
              </div>
              <h3 className="text-xl font-heading font-bold text-gray-900 dark:text-white mb-2">
                Establishing Quantum Link...
              </h3>
              <p className="text-sm max-w-sm text-center text-gray-600 dark:text-gray-400">
                Our AI agent is currently fetching and triaging your inbox. This
                  ...
                  >
                    ...
                    <div className="flex justify-between items-start mb-2">
                      <div className="flex items-center gap-3">
                        ...
                        <span
                          className={`text-sm line-clamp-1 max-w-[150px] ${isUnread ? "font-bold text-gray-900 dark:text-white" : "font-semibold text-gray-700 dark:text-gray-300"}`}
                        >
                          {thread.latestSender
                            .split("<")[0]
                            .replace(/"/g, "")
                      ...
                      <span
                        ...
                        className={`text-xs whitespace-nowrap shrink-0 ml-2 flex items-center gap-1.5 ${isUnread ? "font-bold text-[#FF9494]" : "text-gray-500 dark:text-gray-400"}`}
                      >
                    ...
                    <h4
                      className={`text-sm mb-1 line-clamp-1 pr-6 ${isUnread ? "font-bold text-gray-900 dark:text-white" : "font-medium text-gray-700 dark:text-gray-300"}`}
                    >
                      {thread.subject}
                    </h4>

                    <p
                      className={`text-xs line-clamp-2 leading-relaxed ${isUnread ? "font-medium text-gray-700 dark:text-gray-300" : "text-gray-500 dark:text-gray-500"}`}
                    >
